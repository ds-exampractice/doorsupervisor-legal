(function () {
  ...
  const THEMES = {
    dark:  { '--bg':'#0a0e1a','--bg-soft':'#0e1322','--bg-elev':'#131a2c','--panel':'rgba(255,255,255,0.03)','--border':'rgba(255,255,255,0.09)','--border-strong':'rgba(255,255,255,0.17)','--text':'#eef1f7','--text-mut':'#9aa4ba','--text-dim':'#66708a','--accent':'#5c8fd6','--accent-strong':'#79a6e6','--accent-soft':'rgba(92,143,214,0.13)','--shadow':'0 24px 60px -28px rgba(0,0,0,0.75)','--grid':'rgba(255,255,255,0.045)' },
    light: { '--bg':'#f5f6f9','--bg-soft':'#ffffff','--bg-elev':'#ffffff','--panel':'rgba(16,24,40,0.015)','--border':'rgba(16,24,40,0.10)','--border-strong':'rgba(16,24,40,0.20)','--text':'#111726','--text-mut':'#51607a','--text-dim':'#8893a8','--accent':'#2f5fa3','--accent-strong':'#234d8a','--accent-soft':'rgba(47,95,163,0.07)','--shadow':'0 20px 44px -26px rgba(16,24,40,0.32)','--grid':'rgba(16,24,40,0.05)' }
  };
  ...
  const ITEMS = [
    ...
    { k:'roles',    label:'Job Roles',    href:'JobRoles.html' },
    { k:'privacy',  label:'Privacy',      href:'privacy-policy.html' },
    { k:'terms',    label:'Terms',        href:'terms-and-conditions.html' },
    { k:'deletion', label:'Data Deletion',href:'data-deletion.html' },
  ...
  const CSS = `
    ...
    * { box-sizing: border-box; }
    ...
    .sn-cta { transition: transform .2s; }
  ...
  class SiteNav extends HTMLElement {
    ...
    _render() {
      ...
      const mobLinks = ITEMS.map(it =>
        `<a href="${it.href}" class="sn-mob-link" style="display:block;color:${active===it.k?'var(--text)':'var(--text-mut)'};font-size:15px;font-weight:500;padding:13px 4px;border-bottom:1px solid var(--border)">${it.label}</a>`
      ).join('');
      ...
      this.shadowRoot.innerHTML = `
        <style>${CSS}</style>
        <header style="position:sticky;top:0;z-index:1000;background:color-mix(in srgb,var(--bg) 82%,transparent);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border-bottom:1px solid var(--border)">
          <div style="max-width:1200px;margin:0 auto;padding:0 24px;height:64px;display:flex;align-items:center;justify-content:space-between;gap:16px">
            <a href="Home.html" style="display:flex;align-items:center;gap:12px;flex:none">
